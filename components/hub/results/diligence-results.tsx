...
export function DiligenceResults({ data }: { data: Extract<AgentResult, { agentId: "synthesis" }> }) {
  ...
  if (!dueDiligence.length) return <p className="text-xs text-muted-foreground">No due diligence questions generated.</p>;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        <h3 className="text-sm font-semibold text-[#1A1A2E]">Due Diligence Questions</h3>
      ...
      <div className="space-y-2">
        {dueDiligence.map((section: DDSection) => (
          <div key={section.category} className="rounded-lg bg-[#F8F9FA] border border-border/40 overflow-hidden">
            <button
              onClick={() => setExpandedCategory(expandedCategory === section.category ? null : section.category)}
              className="flex items-center justify-between w-full px-4 py-3 text-left hover:bg-[#F1F5F9] transition"
            >
              <div className="flex items-center gap-3">
                <span className="text-xs font-semibold text-[#1A1A2E]">{section.category}</span>
                <span className="text-[10px] text-muted-foreground">{section.questions.length} questions</span>
              </div>
              <span className="text-muted-foreground text-xs">{expandedCategory === section.category ? "−" : "+"}</span>
            </button>
            ...
            {expandedCategory === section.category && (
              <div className="px-4 pb-3 space-y-3">
                {section.questions.map((q, i) => {
                  ...
                  return (
                    <div key={i} className="rounded-lg bg-[#FAFAFA] border border-border/40 p-3 space-y-2">
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-xs font-medium text-[#1A1A2E] leading-relaxed">{q.question}</p>
                        <span
                      ...
                      <p className="text-[11px] text-muted-foreground leading-relaxed">{q.rationale}</p>
                    </div>
                  );
                })}
              </div>
